...
vector<goal_node*> goal_planner(Map* map, Robot* r, int* open_counter){
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[]){
    ...
    int* open_counter = (int*)malloc(sizeof(int)*1);
    *open_counter = 0;
    vector<goal_node*> goal_plan = goal_planner(m,r,open_counter);
    double sym_planner_time = (( std::clock() - start_clock ) / (double) CLOCKS_PER_SEC);
    cout << "Goal_planner done\n";
    vector<double> x_plan;
    vector<double> y_plan;
    std::clock_t path_clock;
    ...
    double path_planner_time = (( std::clock() - start_clock ) / (double) CLOCKS_PER_SEC);
    cout << "Final A* done\n";
    ...
    memcpy(mxGetPr(plhs[2]), values, sizeof(double)*goal_plan.size()*3);

    plhs[3] = mxCreateDoubleMatrix(1, 4, mxREAL);
    double* times = (double*)malloc(sizeof(double)*4);
    times[0] = sym_planner_time;
    times[1] = path_planner_time;
    times[2] = (( std::clock() - start_clock ) / (double) CLOCKS_PER_SEC);
    times[3] = *open_counter;
    memcpy(mxGetPr(plhs[3]), times, sizeof(double)*4);
